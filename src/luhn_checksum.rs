...
pub fn is_valid(code: &str) -> bool {
    let trimmed = code.split(' ').collect::<String>();
    
    if trimmed.len() <= 1 {
        return false;
    }
    println!("trimmed: {}", trimmed);
    if let Ok(_number) = trimmed.parse::<u64>() {
        let mut total = 0;
        for (idx, c) in trimmed.chars().into_iter().rev().enumerate() {
            if idx % 2 != 0 {
                let mut doubled = c.to_digit(10).unwrap() * 2;
                if doubled > 9 {
                     doubled = doubled - 9;
                }
                total += doubled;
                ...
                println!("{} - {}", c.to_digit(10).unwrap(), total );
            }
...
mod tests {
    ...
    fn process_valid_case(number: &str, is_luhn_expected: bool) {
        let b_is_valid = is_valid(number);

        assert_eq!(b_is_valid, is_luhn_expected);
    }
    #[test]
